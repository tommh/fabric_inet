...
class FabricCSVProcessor:
    ...
    def read_and_clean_csv(self, file_path: str) -> DataFrame:
        ...
        try:
            ...
            # Use multiple columns for ordering to ensure uniqueness, add monotonically_increasing_id for final uniqueness
            cleaned_df = cleaned_df.withColumn('temp_unique_id', monotonically_increasing_id())
            window_spec = Window.orderBy('Attestnummer', 'ImportDato', 'temp_unique_id')
            df_with_row_num = cleaned_df.withColumn('row_num', row_number().over(window_spec))
            cleaned_df = df_with_row_num.withColumn('ImpHist_ID', (col('row_num') + max_id).cast('long')).drop('row_num', 'temp_unique_id')
    ...
    def insert_to_delta_table(self, df: DataFrame, skip_duplicates: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                table_exists = self.spark.catalog.tableExists("energylabels_bronze", "raw_ingestion", "EnovaApi_ImpHist")
                ...
                if table_exists:
                    ...
                    df_to_insert.write \
                        .format("delta") \
                        .mode("overwrite") \
                        .option("path", f"Tables/energylabels_bronze/raw_ingestion/EnovaApi_ImpHist") \
...
# Debug cell - let's see what's happening step by step
processor = FabricCSVProcessor()

# Step 1: Read and examine the raw data
file_path = processor.get_csv_file_path(2009)
print(f"File path: {file_path}")

# Step 2: Read the raw CSV to see what we're working with
raw_df = (processor.spark.read
          .option("header", "true")
          .csv(file_path))

print(f"\nRaw CSV columns: {raw_df.columns}")
print(f"Raw CSV count: {raw_df.count()}")

# Step 3: Check a few sample rows
print("\nSample raw data:")
raw_df.show(3, truncate=False)

# Step 4: Process through cleaning
cleaned_df = processor.read_and_clean_csv(file_path)
print(f"\nCleaned data count: {cleaned_df.count()}")
print(f"Cleaned columns: {cleaned_df.columns}")

# Step 5: Check for null Attestnummer (this might be filtering out records)
null_attestnummer_count = cleaned_df.filter(col('Attestnummer').isNull()).count()
print(f"\nRecords with null Attestnummer: {null_attestnummer_count}")

# Step 6: Show some cleaned data
print("\nSample cleaned data:")
cleaned_df.select('Attestnummer', 'GateAdresse', 'Postnummer', 'Energikarakter').show(5, truncate=False)

# Step 7: Check the filtering logic
filtered_df = cleaned_df.filter(
    col('Attestnummer').isNotNull() | 
    col('GateAdresse').isNotNull() |
    col('Postnummer').isNotNull()
)
print(f"\nAfter filtering: {filtered_df.count()} rows")
...
# Debug the table insertion step
processor = FabricCSVProcessor()

# Get the cleaned data
cleaned_df = processor.read_and_clean_csv(processor.get_csv_file_path(2009))
print(f"Ready to insert: {cleaned_df.count()} rows")

# Check the schema alignment
print(f"\nTarget table columns: {[field.name for field in processor.schema.fields]}")
print(f"DataFrame columns: {cleaned_df.columns}")

# Test the duplicate check
duplicate_check = processor.check_existing_records(cleaned_df)
print(f"\nDuplicate check results:")
print(f"- Existing count: {duplicate_check['existing_count']}")
print(f"- New count: {duplicate_check['new_count']}")
print(f"- New records DataFrame count: {duplicate_check['new_records_df'].count()}")

# Check if there are any errors in the duplicate check
if 'check_error' in duplicate_check:
    print(f"- Check error: {duplicate_check['check_error']}")

# Try the actual insert step manually
print(f"\nAttempting table insert...")
insert_result = processor.insert_to_delta_table(cleaned_df, skip_duplicates=True)
...
# Debug: Let's try creating the table with a simple name first
processor = FabricCSVProcessor()

# Override with a simple table name for testing
processor.table_name = "EnovaApi_ImpHist_Test"

# Get the cleaned data
cleaned_df = processor.read_and_clean_csv(processor.get_csv_file_path(2009))
print(f"Data ready: {cleaned_df.count()} rows")

# Try direct table creation with simple name
try:
    print("Attempting to create table with simple name...")
    cleaned_df.write \
        .format("delta") \
        .mode("overwrite") \
        .option("overwriteSchema", "true") \
        .saveAsTable("EnovaApi_ImpHist_Test")
    print("✅ Success! Table created")
    
    # Verify the data was written
    verification_df = spark.table("EnovaApi_ImpHist_Test")
    row_count = verification_df.count()
    print(f"✅ Verification: {row_count} rows in table")
    
    # Show a few sample rows
    print("\nSample data from table:")
    verification_df.select("ImpHist_ID", "Attestnummer", "GateAdresse", "Energikarakter").show(5)
...
# Let's see what tables exist in the current database
print("Available tables:")
spark.sql("SHOW TABLES").show()

# Or try the full schema name that was originally intended
try:
    df_check = spark.table("energylabels_bronze.raw_ingestion.EnovaApi_ImpHist")
    print(f"Found table with full name! Rows: {df_check.count()}")
    df_check.select("ImpHist_ID", "Attestnummer", "GateAdresse", "Energikarakter").show(5)
except:
    print("Full schema name didn't work either")
    
    # Check what the processor actually used as table name
    processor = FabricCSVProcessor()
    print(f"Processor table name after creation: {processor.table_name}")
    
    # Try to find tables with 'Enova' in the name
    all_tables = spark.sql("SHOW TABLES").collect()
    for table in all_tables:
        table_name = table['tableName']
        if 'Enova' in table_name or 'enova' in table_name:
            print(f"Found Enova table: {table_name}")
...
# Drop the existing table and start over
spark.sql("DROP TABLE IF EXISTS `energylabels_bronze.raw_ingestion.EnovaApi_ImpHist`")

# Process 2009 again with the fixed script
result = process_single_year(2009)
print(f"Fresh start - Inserted: {result['total_inserted']}")

# Verify the IDs are now sequential
df = spark.table("energylabels_bronze.raw_ingestion.EnovaApi_ImpHist")
df.select("ImpHist_ID", "Attestnummer").orderBy("ImpHist_ID").show(10)
...
print("ImpHist_ID distribution:")
df.groupBy("ImpHist_ID").count().orderBy("ImpHist_ID").show(20)

# Check if there are patterns in the data causing this
print("Sample data ordered by ImpHist_ID:")
df.select("ImpHist_ID", "Attestnummer", "ImportDato").orderBy("ImpHist_ID", "Attestnummer").show(20, truncate=False)
...
# Force drop the table completely
spark.sql("DROP TABLE IF EXISTS `energylabels_bronze.raw_ingestion.EnovaApi_ImpHist`")
...
try:
    df_test = spark.table("energylabels_bronze.raw_ingestion.EnovaApi_ImpHist")
    print(f"ERROR: Table still exists with {df_test.count()} rows")
except:
    print("Good: Table successfully dropped")

# Now process 2009 fresh
result = process_single_year(2009)
print(f"Fresh 2009 - Inserted: {result['total_inserted']}")
...
# Alternative: Recreate table without 2009 records
df = spark.table("energylabels_bronze.raw_ingestion.EnovaApi_ImpHist")

# Filter out 2009 records based on certificate date
df_without_2009 = df.filter(year("Utstedelsesdato") != 2009)

print(f"Original count: {df.count()}")
print(f"After removing 2009: {df_without_2009.count()}")
print(f"Removed: {df.count() - df_without_2009.count()} records")

# Recreate table without 2009 data
df_without_2009.write \
    .format("delta") \
    .mode("overwrite") \
    .option("overwriteSchema", "true") \
    .saveAsTable("energylabels_bronze.raw_ingestion.EnovaApi_ImpHist")

print("Table recreated without 2009 data")

# Verify the years remaining
df_final = spark.table("energylabels_bronze.raw_ingestion.EnovaApi_ImpHist")
df_final.select(year("Utstedelsesdato").alias("certificate_year")) \
    .groupBy("certificate_year") \
    .count() \
    .orderBy("certificate_year") \
    .show()
...
# Check if 2009 data exists
df_2009 = df.filter(year("Utstedelsesdato") == 2009)
print(f"2009 records in table: {df_2009.count()}")

# Check latest ImportDato to see if new data was added
df.select("ImportDato").orderBy(col("ImportDato").desc()).show(1)

# Check ImpHist_ID distribution to see if the partitioning issue is fixed
print("ImpHist_ID distribution (checking for duplicate issue):")
df.groupBy("ImpHist_ID").count().filter("count > 1").show(5)
...
try:
    ...
    file_path = processor.get_csv_file_path(2009)
    print(f"File exists: {processor.check_file_exists(file_path)}")
    
    # Step 2: Try the CSV analysis
    analysis = processor.analyze_csv_structure(file_path)
    print(f"Analysis successful: {analysis['total_rows']} rows")
    
    # Step 3: Try reading and cleaning (this is likely where it fails)
    cleaned_df = processor.read_and_clean_csv(file_path)
    print(f"Cleaning successful: {cleaned_df.count()} rows")
